...
def create_paper_check_agent(settings: Settings, tool_choice: str | None = None) -> Agent:
    retrieve_academic_papers = create_retrieve_academic_papers_tool(settings)
    return Agent(
        name="PaperCheckAgent",
        handoff_description=(
            "Academic manuscript checking, review-style diagnosis, evidence sufficiency "
            "assessment, logic/concept/tone evaluation, and optional user-authorized patch generation."
        ),
        instructions=build_paper_check_agent_instructions(),
        model=create_deepseek_model(settings),
        model_settings=ModelSettings(tool_choice=tool_choice),
        tools=[*CHECKING_AGENT_TOOLS, retrieve_academic_papers],
    )
...
def create_paper_agent(settings: Settings, tool_choice: str | None = None) -> Agent:
    ...
    return Agent(
        ...
        handoffs=[
            handoff(
                paper_check_agent,
                tool_name_override="handoff_to_paper_check_agent",
