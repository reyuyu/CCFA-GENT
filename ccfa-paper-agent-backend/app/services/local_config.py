...
def _write_env(values: dict[str, str], path: Path = ENV_PATH) -> None:
    preferred_order = [
        ...
        "DEEPSEEK_BASE_URL",
        "DEEPSEEK_MODEL",
        "LATEX_DRAFT_CLEAN_MODEL",
        "LATEX_DRAFT_CLEAN_CONCURRENCY",
    ...
    ENV_PATH.write_text(
        "\n".join(
            [
                ...
                "LATEX_DRAFT_CLEAN_MODEL=deepseek-v4-flash",
                "LATEX_DRAFT_CLEAN_CONCURRENCY=3",
...
def _response_from_settings(settings: Settings, saved: bool = True) -> LocalConfigResponse:
    return LocalConfigResponse(
        ...
        latexDraftCleanModel=settings.latex_draft_clean_model,
